...
describe("home showcase preview", () => {
  ...
  it("keeps translated labels selectable", () => {
    ...
    expect(getCopy("ko").scanner.runScan).toBe("내 런치 점수 받기")
    expect(getCopy("es").scanner.runScan).toBe("Obtener mi score")
    expect(getCopy("ja").scanner.runScan).toBe("ローンチスコアを取得")
